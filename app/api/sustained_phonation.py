...
from app.services.audio.sustained_phonation import SustainedPhonationAnalyzer
from app.services.laravel_client import send_result_to_laravel, LaravelClientError

router = APIRouter(
    prefix="/api/sustained-phonation",
    tags=["Sustained Phonation"]
)


@router.post("/analyze")
async def analyze_sustained_phonation(
    patient_id: int = Form(...),
    session_id: str = Form(...),
    file: UploadFile = File(...)
):
    """
    Upload a sustained-vowel recording, measure intensity and
    duration, and store the result via Laravel.
    """
    ...
    try:
        analyzer = SustainedPhonationAnalyzer(
            filepath=filepath,
    ...
    return {
        "success": True,
        "message": "Sustained phonation exercise recorded successfully."
    }